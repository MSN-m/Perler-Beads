...
export function downloadMirroredImage() {
    ...
    mirroredPixelData.forEach(p => {
        if (!p || p.id === 'NONE') return; // 确保颜色对象存在
        if (!stats[p.id]) stats[p.id] = { ...p, count: 0 };
        stats[p.id].count++;
    });
    const sortedStats = Object.values(stats).sort((a, b) => b.count - a.count);
    ...
    for (let y = 0; y < AppState.gridHeight; y++) {
        for (let x = 0; x < AppState.gridWidth; x++) {
            const i = (y * AppState.gridWidth + x);
            if (mirroredPixelData[i] && mirroredPixelData[i].id !== 'NONE') {
                if (x < minX) minX = x;
                if (x > maxX) maxX = x;
                if (y < minY) minY = y;
                if (y > maxY) maxY = y;
    ...
    const title = `颜色清单(${brandText} 镜像)-共${sortedStats.length}种颜色`; // 修改标题
    exportCtx.fillText(title, padding, statsStartY - 60);
    ...
    sortedStats.forEach((c, index) => {
        ...
        exportCtx.font = 'bold 32px Arial';
        exportCtx.textAlign = 'left';
        exportCtx.fillText(`${c.id} (镜像)`, x + 30, y + cardHeight / 2); // 修改色号显示
