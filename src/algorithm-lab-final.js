...
function settings() { const side = Math.max(16, Math.min(104, Number($('final-grid-size').value) || 100)); const ratio = sourceImageData.width / Math.max(1, sourceImageData.height); return { width: ratio >= 1 ? side : Math.max(16, Math.round(side * ratio)), height: ratio >= 1 ? Math.max(16, Math.round(side / ratio)) : side, brand: $('final-brand').value, maxColors: Math.max(4, Math.min(80, Number($('final-max-colors').value) || 12)) }; }
...
function draw(canvas, pixels, width, height) { canvas.width = width; canvas.height = height; canvas.style.aspectRatio = `${width} / ${height}`; canvas.style.width = width >= height ? '100%' : 'auto'; canvas.style.height = width >= height ? 'auto' : '100%'; const context = canvas.getContext('2d'), image = context.createImageData(width, height); pixels.forEach((p, i) => { const o = i * 4; image.data[o] = p.r; image.data[o + 1] = p.g; image.data[o + 2] = p.b; image.data[o + 3] = p.a ?? 255; }); context.putImageData(image, 0, 0); }
